...
def build_parser() -> argparse.ArgumentParser:
    ...
    browse.add_argument("--pages", type=int, default=8, help="最多滚动采集多少屏（默认 8）")
    ...
    browse.add_argument(
        ...
        type=int,
        default=2,
        help="连续多少屏没有新帖子后停止（默认 2）",
    )
    browse.add_argument(
        "--settle-seconds",
        type=float,
        default=1.8,
        help="每次页面操作后等待秒数（默认 1.8）",
    )
    return parser
...
def browse(args: argparse.Namespace) -> int:
    if args.pages < 1 or args.stop_after_stale_pages < 1:
        raise AdbError("--pages 和 --stop-after-stale-pages 必须大于 0。")
    client = create_client(args)
    agent = BrowseAgent(client, args.data_dir.resolve(), settle_seconds=args.settle_seconds)
    result = agent.browse(
        pages=args.pages,
        latest=not args.keep_sort,
        screenshots=not args.no_screenshots,
        stop_after_stale_pages=args.stop_after_stale_pages,
    )
